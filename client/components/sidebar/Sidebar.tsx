/**
 * components/sidebar/Sidebar.tsx — FIXED: onHideConv
 */
'use client';
...
interface SidebarProps {
  ...
  onHideConv: (convId: string) => void; onLogout: () => void; onEditProfile?: () => void;
}

export function Sidebar({ user, groups, conversations, activeGroupId, activeConvId, onSelectGroup, onSelectConv, onHideConv, onLogout, onEditProfile }: SidebarProps) {
  ...
  return (
    <div className="w-[250px] glass-strong rounded-2xl flex flex-col overflow-hidden flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto px-1 py-1">
        <div className="px-3 pt-3 pb-1.5 text-[9px] font-semibold text-[var(--t3)] uppercase tracking-[.12em]">Salons publics</div>
        {groups.map(g => (
          <button key={g.id} onClick={() => onSelectGroup(g)}
            className={`w-full flex items-center gap-2.5 px-3 py-2 mx-2 rounded-lg text-sm transition-all border border-transparent
              ${activeGroupId === g.id && !activeConvId ? 'bg-[var(--glass-a)] border-[rgba(139,92,246,0.2)] text-[var(--t1)] shadow-[0_0_20px_rgba(139,92,246,0.06)]' : 'text-[var(--t2)] hover:bg-[var(--glass-h)] hover:border-[var(--border)] hover:text-[var(--t1)]'}`}>
            <span className="text-base font-bold text-[var(--hash)] w-[22px] text-center" style={{ textShadow: '0 0 12px var(--acc-g)' }}>#</span>
            <span className="flex-1 font-medium text-left">{g.name}</span>
            {g.member_count > 0 && <span className="text-[10px] text-[var(--t3)]">{g.member_count}</span>}
          </button>
        ))}
